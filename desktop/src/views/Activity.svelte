<script lang="ts">
	...
	function groupByDay(
		...
		return Array.from(out.entries()).map(([day, items]) => ({
			day,
			items,
		}));
	}
...
<div class="activity">
	...
	<div class="filters">
		...
		<label>
			...
			<select
				value={actorFilter}
				on:change={(e) => applyActorFilter(e.currentTarget.value)}
			>
	...
	{#if filtered.length === 0}
		<div class="empty">
			...
			<p>
				Activity appears here when you scan, open, or modify vobes. As
				you use Vobes, this fills up.
			</p>
		</div>
	{:else}
