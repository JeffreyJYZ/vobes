...
import type { ActivityEvent, TodoHit } from "../lib/types"
...
async function doOpenInEditor(v: (typeof $vobes)[number]) {
	...
	try {
		...
		await api.openInEditor(v.name, v.path)
		pushToast({ kind: "success", message: `Opened ${v.name} in editor.` })
	} catch (e) {
...
async function doOpenTerminal(v: (typeof $vobes)[number]) {
	...
	try {
		await api.markOpened(v.name)
		await api.openInTerminal(v.name)
...
async function doCopyContextPack(v: (typeof $vobes)[number]) {
	...
	try {
		...
		pushToast({ kind: "error", message: `Context pack: ${errorString(e)}` })
	} finally {
		copyingPack = false
	}
}
...
async function addTag() {
	...
	busy = true
	try {
		const fresh = await api.setTags(selected.name, [...selected.tags, t])
		selectedVobe.set(fresh)
		newTag = ""
	} catch (e) {
		pushToast({ kind: "error", message: errorString(e) })
  ...
  {#if !selected}
    <div class="empty-state">
      <div class="empty-card">
        ...
        <p>
          The Dashboard is the fastest way to find something — search, sort,
          or scan the attention section.
        </p>
        <div class="empty-actions">
          <button class="primary" on:click={() => view.set("dashboard")}>
            Go to Dashboard
          </button>
        </div>
        <div class="empty-hint">
          Or press <kbd>2</kbd> to jump straight here, <kbd>⌘K</kbd> to fuzzy-find a vobe.
        </div>
      </div>
    </div>
  {:else}
    <div class="head">
      ...
      <div class="row">
        ...
        <button on:click={() => doCopyPath(selected)} disabled={busy}>Copy path</button>
        <button on:click={() => doReveal(selected)} disabled={busy}>Reveal</button>
        <button on:click={() => doOpenTerminal(selected)} disabled={busy}>Terminal</button>
        <button on:click={() => doCopyContextPack(selected)} disabled={busy || copyingPack}>
          {copyingPack ? "Copying…" : "Copy context pack"}
        </button>
        <button class="primary" on:click={() => doOpenInEditor(selected)} disabled={busy}>
          Open in editor
        </button>
      </div>
    </div>
    ...
    <div class="detail">
      <section class="card">
        <div class="kv">
          ...
          <div class="v" class:vanilla={!selected.language}>{languageLabel(selected.language)}</div>
          <div class="k">Framework</div>
          <div class="v" class:vanilla={!selected.framework}>{frameworkLabel(selected.framework)}</div>
          <div class="k">Package manager</div>
          <div class="v" class:vanilla={!selected.package_manager}>{packageManagerLabel(selected.package_manager)}</div>
        ...
        <section class="card">
          ...
          <div class="kv">
            ...
            <div class="v">
              ...
              {#if selected.git.ahead > 0} · ↑{selected.git.ahead} to push{/if}
              {#if selected.git.behind > 0} · ↓{selected.git.behind} behind{/if}
            </div>
            {#if selected.git.last_commit}
              <div class="k">Last commit</div>
              <div class="v">
                <div class="mono small">{selected.git.last_commit.hash.slice(0, 7)}</div>
                <div>{selected.git.last_commit.message}</div>
                <div class="muted small">
                  {selected.git.last_commit.author} · {relativeTime(selected.git.last_commit.date)}
                </div>
              </div>
            {/if}
          </div>
        ...
        {#if showTodos && todos.length > 0}
          <ul class="todos">
            {#each todos.slice(0, 20) as t (t.file + ":" + t.line + t.text)}
              <li>
                <span class="kind kind-{t.kind.toLowerCase()}">{t.kind}</span>
                <span class="loc mono">{t.file}:{t.line}</span>
                <span class="text">{t.text}</span>
              </li>
            {/each}
            {#if todos.length > 20}
              <li class="muted small">… and {todos.length - 20} more</li>
            {/if}
          </ul>
        {/if}
      </section>
      ...
      <section class="card span2">
        <div class="row-between">
          ...
          {#if readmeLoaded && readmeText}
            <div class="readme-toggle" role="tablist" aria-label="README view">
              <button
                type="button"
                role="tab"
        ...
        {#if readmeLoaded}
          {#if !readmeText}
            <div class="muted small">No README found in this project.</div>
          {:else if readmeMode === "preview"}
            <!-- svelte-ignore a11y-no-noninteractive-element-interactions -->
            <!-- svelte-ignore a11y-click-events-have-key-events -->
            <!-- eslint-disable-next-line svelte/no-at-html-tags -->
            <div class="md" on:click={openReadmeLink} role="document">
              {@html readmeHtml}
            </div>
          {:else}
      ...
      <section class="card span2">
        ...
        ></textarea>
        <div class="actions">
          <button on:click={() => (notesDraft = selected.notes ?? "")}>Reset</button>
          <button class="primary" on:click={saveNotes} disabled={savingNotes}>
      ...
      <section class="card span2 danger-zone">
        <h3>Danger zone</h3>
        <button class="danger" on:click={() => removeVobe(selected)} disabled={busy}>
          Untrack this project
        </button>
        <div class="muted small">
          Vobes forgets this project but does not touch the files on disk.
        </div>
      </section>
    </div>
  {/if}
...
<style>
  ...
  .head h2 { margin: 0; flex: 1; min-width: 160px; }
  ...
  .link {
    ...
    color: var(--accent); padding: 0; font: inherit;
    text-decoration: underline; font-size: 12px;
  }
  .kv {
    display: grid; grid-template-columns: 130px 1fr; gap: 8px 16px;
  }
  .k { color: var(--fg-muted); font-size: 12.5px; }
  .v { font-size: 13.5px; }
  .v.mono { font-family: ui-monospace, Menlo, monospace; font-size: 12.5px; }
  .small { font-size: 12px; }
  .muted { color: var(--fg-muted); }
  .vanilla { color: var(--fg-faint); }
  .tag {
  ...
  .timeline .when { color: var(--fg-faint); min-width: 80px; }
  .timeline .what { font-weight: 600; }
  .timeline .detail { color: var(--fg-muted); }
  .readme {
    margin: 0;
    padding: 12px 14px;
    background: var(--bg-sunken);
  ...
  .md :global(h3) { font-size: 14px; font-weight: 700; }
  .md :global(h4) { font-size: 13px; font-weight: 600; color: var(--fg-muted); }
  ...
  .kind-todo { background: color-mix(in srgb, var(--accent) 18%, transparent); color: var(--accent); }
  .kind-fixme { background: color-mix(in srgb, var(--danger) 18%, transparent); color: var(--danger); }
  .kind-xxx { background: color-mix(in srgb, var(--warn) 18%, transparent); color: var(--warn); }
  .todos .loc {
    color: var(--fg-faint); font-size: 11.5px;
    overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
    max-width: 260px;
  }
  .todos .text { color: var(--fg); flex: 1; min-width: 0; }
