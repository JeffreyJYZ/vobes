	...
	function filterAndSort(all: Vobe[], q: string, key: SortKey): Vobe[] {
		...
		return [...arr].sort((a, b) => {
			...
			switch (key) {
				...
				case "last_opened":
					return (b.last_opened ?? "").localeCompare(
						a.last_opened ?? "",
					);
				default:
...
<div class="dashboard">
	...
	<div class="toolbar">
		<div class="left">
			<h2>Dashboard</h2>
			<span class="muted">
				{$vobes.length} vobe{$vobes.length === 1 ? "" : "s"} · updated {relativeTime(
					toISOStr($lastRefreshed),
				)}
			</span>
		</div>
		<div class="right">
			...
			<button
				...
				on:click={async () => {
					const ok = await ask(
						"Reset and re-scan? This erases every tracked vobe + all activity, then rediscovers from your roots.",
				...
				disabled={$scanning}
				title="Purge every vobe and re-scan from scratch"
			>
				Reset
			</button>
			<button
	...
	{#if $vobes.length === 0}
		<div class="empty">
			...
			<p>
				Configure scan roots in <button
					class="link"
					on:click={() => view.set("settings")}>Settings</button
		...
		<div class="empty">
			...
			<p>
				Try a different search or <button
					class="link"
					on:click={() => searchQuery.set("")}>clear it</button
				>.
			</p>
		</div>
	{:else}
