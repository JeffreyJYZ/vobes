...
{#if isOpen}
	<div class="backdrop" role="presentation" on:click={backdropClick}>
				...
				{:else}
					{#if projectResults.length > 0}
						...
						{#each projectResults as r, i (r.id)}
							...
							>
								<span class="kind-dot git" title="Project"
								></span>
								<span class="text">
					...
					{#if commandResults.length > 0}
						...
						{#each commandResults as r, i (r.id)}
							...
							<button
								...
								on:mouseenter={() => (active = idx)}
							>
								<span class="kind-dot cmd" title="Command"
									>›</span
								>
