      ...
      {#each items as it (it.vobe.id)}
        <li>
          <button class="item" on:click={() => open(it.vobe)}>
            ...
            <span class="when">{relativeTime(it.vobe.last_modified)}</span>
          </button>
        </li>
      {/each}
    </ul>
...
<style>
  ...
  .bar[data-w="1"] { background: var(--fg-faint); }
  .bar[data-w="2"] { background: var(--accent); }
  .bar[data-w="3"] { background: var(--warn); }
  ...
  .when { color: var(--fg-faint); font-size: 12px; }
</style>
